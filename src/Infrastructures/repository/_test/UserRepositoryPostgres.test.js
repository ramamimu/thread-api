...
describe("UserRepositoryPostgres", () => {
  ...
  describe("verifyAvailableUserId function", () => {
    it("should throw InvariantError when user not exist", async () => {
      // Arrange
      // not needed spesific arrangement
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});

      // Action & Assert
      await expect(
        userRepositoryPostgres.verifyAvailableUserId("user-11111")
      ).rejects.toThrowError(InvariantError);
    });

    it("should not throw InvariantError when username available", async () => {
      // Arrange
      await UsersTableTestHelper.addUser({ id: "user-1112" });
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});

      // Action & Assert
      await expect(
        userRepositoryPostgres.verifyAvailableUserId("user-1112")
      ).resolves.not.toThrowError(InvariantError);
    });
  });
